...
function runDaemon() {
  ...
  try {
    ...
    // Early exit: check if any source directories changed since last sync
    if (hasSourcesChangedSinceLastSync()) {
      // Step 1: Stage local session files into staging directory
      ...
      // Step 2: Scan for secrets before committing
      if (result.files > 0) {
      ...
      // Step 3: Commit local changes first, then pull+merge remote, then push.
      // This lets git handle merge conflicts properly instead of filesystem
      // mtime comparisons. The pull uses -X theirs so remote data from other
      // machines is preserved on conflict (last push wins).
      GitManager.sync();
    } else {
      Logger.log('INFO', 'No source changes since last sync — skipped.');
    }
  } catch (error) {
...
const COMMANDS = {
  ...
  untag: (args) => {
    ...
    if (!tagName) {
      console.log('Usage: node sync-engine.js untag <tag-name>');
      return;
    }
