...
// ─────────────────────────────────────────────────────────────────────────────
// npm postinstall: show a welcome message pointing users to `synctx setup`.
// npm v7+ suppresses lifecycle script output, so the full interactive setup
// runs on first `synctx` use (via firstRunSetup in sync-engine.js) or by
// running `synctx setup` explicitly.
// The full setup logic below still runs when invoked directly:
//   node scripts/postinstall.js
//   synctx setup (delegates here via SYNCTX_INSTALLER)
// ─────────────────────────────────────────────────────────────────────────────

const IS_NPM_LIFECYCLE = !!(
  process.env.npm_lifecycle_event ||
  process.env.npm_config_global ||
  process.env.npm_package_json
);

if (IS_NPM_LIFECYCLE && !process.env.SYNCTX_INSTALLER) {
  // Running as npm postinstall — print a short welcome and exit
  console.log('');
  console.log('  ✓ Synctx installed successfully!');
  console.log('');
  console.log('  Get started:');
  console.log('    synctx setup      — Interactive first-time setup');
  console.log('    synctx --help     — Show all commands');
  console.log('');
  console.log('  Setup runs automatically on first use of any synctx command.');
  console.log('');
  process.exit(0);
}
...
async function main() {
  ...
  try {
    const ghUser = execSync('gh api user --jq .login', { encoding: 'utf8', windowsHide: true }).trim();
    console.log(`  Sync repo: https://github.com/${ghUser}/${repoName}`);
    console.log('');
  } catch { /* skip if gh not available */ }
  console.log('  Sessions will sync automatically on:');
  console.log('    • Every prompt and tool use');
  console.log('    • Session exit and errors');
  console.log('');
  console.log('  Quick start:');
  console.log('    synctx list              — Show all sessions');
  console.log('    synctx tag current name  — Tag the current session');
  console.log('    synctx restore name      — Restore by tag');
  console.log('    synctx help              — All commands');
  console.log('');
}
