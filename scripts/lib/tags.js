...
function assign(tag, sessionId, cli) {
  ...
  if (tags[normalized] && tags[normalized].sessionId !== sessionId) {
    // Conflict: tag exists for a different session — auto-suffix
    const suffix = `-${os.hostname().toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 8)}`;
    const altTag = normalized + suffix;
    tags[altTag] = { cli, sessionId, createdAt: new Date().toISOString() };
    writeTags(tags);
    return {
      success: true,
      warning: `Tag "${normalized}" already taken. Assigned as "${altTag}" instead.`,
      assignedTag: altTag,
    };
  }
