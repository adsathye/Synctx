...
function testStatus() {
  ...
  assert('status exits cleanly', result.ok || result.output.includes('Synctx'),
    `exit: ${result.status}`);
  assert('status shows version', result.output.includes('1.0.0'),
...
function testHookSimulation() {
  ...
  assert('hook push completed within timeout', attempts < 30,
...
function testInstallation() {
  ...
  // Verify install.js has valid syntax (don't execute — it runs top-level code)
  const installCheck = spawnSync(process.execPath, ['--check', path.join(PROJECT_ROOT, 'install.js')], {
    cwd: PROJECT_ROOT,
    encoding: 'utf8',
    timeout: 10000,
  });
  assert('install.js has valid syntax', installCheck.status === 0,
...
function testHelpVersion() {
  ...
  assert('--version shows version', versionResult.output.includes('1.0.0'),
...
function main() {
  console.log('\n' + '═'.repeat(70));
  console.log('  Synctx v1.0.0 — E2E Test Suite');
